...
- (UIImage *)blurredImageWithRadius:(CGFloat)radius iterations:(NSUInteger)iterations tintColor:(UIColor *)tintColor
{
    ...
    {
        CGContextSetFillColorWithColor(ctx, [UIColor colorWithRed:70.0/255.0 green:70.0/255.0 blue:70.0/255.0 alpha:0.5].CGColor);
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (GaussianBlurFilter *view in self.views)
        {
...
- (void)addView:(GaussianBlurFilter *)view
{
...
- (void)removeView:(GaussianBlurFilter *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            GaussianBlurFilter *view = self.views[i];
    ...
    if ([self shouldUpdate])
    {
        ...
        if (async)
        {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self setLayerContents:[self blurredSnapshot:[self snapshotOfUnderlyingView] radius:[self blurPresentationLayer].blurRadius]];
